...
<body class=" theme-blue">
    ...
    <script type="text/javascript">
        ...
        function del(id){
        	if(confirm("确定要删除吗？")){
        		window.location.href="<%=path%>/admin_delPhoto?id="+id;
        	}
        }
    </script>
...
<div class="btn-toolbar list-toolbar">
    <button class="btn btn-primary" onclick="javascript:window.location.href='<%=path %>/admin/photoForm.jsp'"><i class="fa fa-plus"></i> New Photo</button>
  <div class="btn-group">
  </div>
</div>
<table class="table">
  ...
  <tbody>
  <s:iterator value="plist" id="pentity" status="st">
    <tr>
      <td><s:property value="#st.index+1"/></td>
      <td><s:property value="name"/></td>
      <td><a href="<%=path%>/home_viewPhoto?id=<s:property value='p_id'/>" target="_blank"><img src="<%=path %>/miniphoto/manage/<s:property value='path'/>" style="width: 160px;height:80px ;"/></a></td>
      
      <td><s:property value="describe"/></td>
      <td><s:property value="path"/></td>
      <td>
          <a href="#"><i class="fa fa-pencil"></i></a>
          <a href="javascript:del(<s:property value='p_id'/>)"><i class="fa fa-trash-o"></i></a>
      </td>
    </tr>
</s:iterator>
	<tr>
		<td colspan="5">
			...
			<form action="<%=path%>/admin_listPhoto" method="post" id="hiform">
