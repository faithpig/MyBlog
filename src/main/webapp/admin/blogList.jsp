...
<body class=" theme-blue">
						...
						$.ajax({
									...
									success:function(data){
											if($.trim(data.say)=="") window.location.href="<%=path%>/admin/login.jsp";
												...
												window.location.href="<%=path%>/admin_listBlog";
											}
									}
								});	
					}
    ...
    <script type="text/javascript">
        ...
        function del(id){
        	if(confirm("确定要删除吗？")){
        		window.location.href="<%=path%>/admin_delBlog?id="+id;
        	}
        }
    </script>
...
<div class="btn-toolbar list-toolbar">
    <button class="btn btn-primary" onclick="javascript:window.location.href='<%=path %>/admin/blogForm.jsp'"><i class="fa fa-plus"></i> New Blog</button>
    <button class="btn btn-primary" id="delAll">批量删除</button><span id="btn_say"></span>
  <div class="btn-group">
  </div>
</div>
<table class="table">
  ...
  <tbody>
  <s:iterator value="blist" id="bentity" status="st">
    <tr>
    <td><input type="checkbox" name="allSelect" value='<s:property value="b_id"/>'/></td>
      <td><s:property value="#st.index+1"/></td>
      <td><a href="<%=path%>/home_viewBlog?id=<s:property value='b_id'/>" target="_blank"><s:property value="title"/></a></td>
      <td><s:date name="createTime" format="yyyy-MM-dd" /></td>
      <td><s:date name="alterTime" format="yyyy-MM-dd" /></td>
      <td>
          <a href=""><i class="fa fa-pencil"></i></a>
          <a href="javascript:del(<s:property value='b_id'/>)"><i class="fa fa-trash-o"></i></a>
      </td>
    </tr>
</s:iterator>
	<tr>
	...
	<form action="<%=path%>/admin_listBlog" method="post" id="hiform">
